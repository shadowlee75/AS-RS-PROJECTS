(function (root, factory) {
  ...
  'use strict';
  const VERSION = '0.3.4', SCHEMA = 'stc/project@1';
  const defaults = {
    ...
    operatingEfficiency: null, A: null, Ft: null, Ew: null, basis: '', basisNote: '',
  ...
  const fields = {
    ...
    operatingEfficiency: ['운전효율', '%', 1, 100, 0.1],
  ...
  const nullable = ['operatingEfficiency', 'A', 'Ft', 'Ew', 'ioZ', 'outX', 'outY', 'outZ', 'firstLevelHeight', 'levelPitch', 'rearStroke'];
  ...
  function migrate(input) {
    ...
    return { ...input, ioZ: input.ioZ ?? null, outX: input.outX ?? null, outY: input.outY ?? null, outZ: input.outZ ?? null,
      ...
      operatingEfficiency: input.operatingEfficiency === undefined ? null : input.operatingEfficiency,
  ...
  function calculate(input) {
    ...
    const factorSource = c.operatingEfficiency !== null ? 'efficiency' : corrected ? 'coefficients' : null;
    const factor = factorSource === 'efficiency' ? c.operatingEfficiency / 100 : corrected ? c.A * c.Ft * c.Ew : null, cache = new Map();
    function one(cfg) {
    ...
    return { ...first, config: c, p, mix: mixing, allRoutes, demand, factor, factorSource, capacity, util, worstUtil, required, theoryTotal, aisles,
  ...
  function correctionText(result) {
    return result.factorSource === 'efficiency' ? '운전효율 ' + result.config.operatingEfficiency + '% 직접 적용' :
      result.factor === null ? '운전효율 또는 세부 계수·근거 입력 필요' : 'A × Fₜ × E𝑤 = ' + Number(result.factor.toFixed(4)) + ' (' + Number((result.factor * 100).toFixed(2)) + '%)';
  }
  function sensitivity(result) {
    const rows = [];
    if (result.factor === null) return rows;
    const keys = result.factorSource === 'efficiency' ? ['operatingEfficiency'] : ['A', 'Ft', 'Ew'];
    for (const key of keys) for (const delta of [-0.1, -0.05, 0.05, 0.1]) {
      const value = Math.max(fields[key][2], Math.min(fields[key][3], result.config[key] * (1 + delta)));
