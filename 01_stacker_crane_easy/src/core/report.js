(function (root, factory) {
  ...
  function markdown(result, sim) {
    ...
    for (const key of Object.keys(Model.defaults).filter(key => key !== 'ports' && key !== 'routes')) {
      ...
      const value = key === 'rackDepth' ? Model.depthName(c) : c[key] === null ? (['firstLevelHeight', 'levelPitch', 'rearStroke'].includes(key) ? '자동' : '미입력') : key === 'transferLift' ? number(c[key] * 1000) : speed ? number(c[key] * (c.speedUnit === 'm/min' ? 60 : 1)) : c[key];
      rows.push('| ' + escape(field?.[0] || (key === 'rackDepth' ? '화물 보관 깊이' : key)) + ' | ' + escape(value) + ' | ' + escape(speed ? c.speedUnit : field?.[1] || '—') + ' |');
    }
    ...
    rows.push('', '## 2. 사이클타임 분해', '', '| 항목 | 시간 (s) |', '| --- | ---: |',
      ...
      '', '- 적용 보정: ' + Model.correctionText(result) + '.',
      ...
      '- 세부 계수 방식에서 A·Ft·Ew 중 1.0인 계수는 해당 손실을 제외하는 이상 조건입니다.', '- SC-stay의 분석값은 SC-return 참고값이며 동작별 결과는 DES에서 확인합니다.',
      '', '## 5. 보정계수 민감도', '', '| 계수 | 상대변화 | 적용값 | 전체 보정능력 | 필요 통로 |', '| --- | ---: | ---: | ---: | ---: |');
    for (const row of Model.sensitivity(result)) rows.push('| ' + (row.key === 'operatingEfficiency' ? '운전효율' : row.key) + ' | ' + number(row.delta * 100) + '% | ' + number(row.value) + (row.key === 'operatingEfficiency' ? '%' : '') + ' | ' + number(row.capacity) + ' | ' + row.required + ' |');
    if (result.factor === null) rows.push('', '운전효율 또는 세부 계수·근거 입력 후 민감도를 계산합니다.');
