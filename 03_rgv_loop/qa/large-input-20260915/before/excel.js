(function(root,factory){if(typeof module==='object'&&module.exports)module.exports=factory(require('./model'),require('./xlsx'));else root.LoopExcel=factory(root.LoopModel,root.LoopXlsx);})(globalThis,function(Model,Xlsx){
  'use strict';
  ...
  const choices={
    ...
    arrival:{takt:'일정 간격 + 변동',poisson:'Poisson 도착'},demandMode:{rate:'OD 물동량 · 시간대 배율',history:'작업이력'}
  };
  const labels={name:'프로젝트명',direction:'운행 방향',distanceUnit:'화면 거리 단위',speedUnit:'화면 속도 단위',unit:'화물 단위',handlingMode:'화면 이재시간 입력',dispatch:'배차',parking:'빈 차량 대기',controlMode:'차량 제어',analysisMode:'거리 분석',etaMode:'간섭 η(N)',basis:'계수 근거',basisNote:'근거 메모',arrival:'작업 도착',demandMode:'수요 입력'};
  const keys=Object.keys(Model.defaults).filter(k=>!Array.isArray(Model.defaults[k]));
  const basicHeaders=['키','항목','입력값','단위','설명'];
  const specs={
    '설비스테이션':{key:'stations',min:1,max:30,headers:['ID','이름','위치 s (m)','총 상차시간 (s)','총 하차시간 (s)','출고 버퍼 (개)','반출 간격 (s/개)'],fields:['id','name','s','load','unload','buffer','takeaway'],widths:[16,24,19,23,23,22,25]},
    'OD물동량':{key:'ods',max:200,headers:['OD ID','출발 ID','도착 ID','요구 물동량 (개/h)'],fields:['id','from','to','rate'],widths:[18,20,20,30]},
    '시간대비율':{key:'profile',max:48,headers:['시작 (min)','끝 (min)','비율 (100%=1배)'],fields:['start','end','multiplier'],widths:[24,24,30]},
    '제한속도':{key:'zones',max:30,headers:['시작 s (m)','끝 s (m)','제한속도 (m/min)'],fields:['from','to','v'],widths:[24,24,30]},
    '작업이력':{key:'history',max:20000,headers:['도착 (s)','출발 ID','도착 ID','수량 (개)'],fields:['time','from','to','quantity'],widths:[24,20,20,24]}
  };
  ...
  function fromSheets(raw){
    const errors=[],fail=(where,message)=>{if(errors.length<30)errors.push(where+': '+message);},blank=x=>x===null||x===undefined||String(x).trim()==='';
    ...
    function table(name,headers,max){
      ...
      if(result.length>max)fail(name,'최대 '+max+'개 행을 입력할 수 있습니다.');return result;
    }
    function number(value,where,min=-Infinity,max=Infinity,integer=false){
      ...
      if(!Number.isFinite(n)||n<min||n>max||integer&&!Number.isInteger(n)){fail(where,(integer?'정수':'숫자')+' '+min+'~'+max+' 범위로 입력하세요.');return NaN;}return n;
    }
    ...
    for(const {row,values:[rawKey,,value,units]}of table('기본조건',basicHeaders,keys.length)){
      ...
      else if(Model.fields[key]){const [,kind,min,max]=Model.fields[key],scale=kind==='speed'?60:1;c[key]=number(value,where,min*scale,max*scale,['vehicles','batchSize','blocks','seed'].includes(key))/scale;}
      else c[key]=String(value??'');
    }
    for(const k of keys)if(!seen.has(k))fail('기본조건','필수 항목 누락: '+k);
    ...
    for(const key of ['ods','history'])checkRows(key,(o,i,w)=>{if(!stationIds.has(o.from))fail(w+' 출발 ID',o.from+' 스테이션이 없습니다.');if(!stationIds.has(o.to))fail(w+' 도착 ID',o.to+' 스테이션이 없습니다.');});
    ...
    if(errors.length)throw Error(errors.join('\n'));
    const config=Model.validate(c);validateExperiment(experiment,config);return {config,experiment};
  }
  const write=(c,e)=>Xlsx.write(sheets(c,e));
  const read=async file=>{if(file.name&&!/\.xlsx$/i.test(file.name))throw Error('Excel 통합 문서(.xlsx) 파일을 선택하세요.');return fromSheets(await Xlsx.read(file));};
