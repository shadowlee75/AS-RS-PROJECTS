(function (root) {
  ...
  class Scene {
    constructor(container, c, onCell) {
      ...
      this.stride = Math.max(5, Model.depthStroke(c, Model.depthCount(c)) * 2 + 2.5); this.depth = (c.cranes - 1) * this.stride;
      ...
      const loadW = Math.min(px * .74, 1.6), loadH = Math.min(py * .6, 1.2), loadD = Math.min(1, c.stroke * .7, Model.depthCount(c) === 2 ? (Model.depthStroke(c, 2) - c.stroke) * .7 : Infinity);
      ...
      this.loads = new T.InstancedMesh(loadGeometry, loadMaterial, c.bays * c.levels * c.sides * Model.depthCount(c) * c.cranes);
      ...
      for (let i = 0; i < c.cranes; i++) {
        ...
        for (let side = 0; side < c.sides; side++) for (let depth = 1; depth <= Model.depthCount(c); depth++) {
          const z = z0 + (side === 0 ? -1 : 1) * Model.depthStroke(c, depth);
          ...
          this.label((side === 0 ? 'A' : 'B') + (Model.depthCount(c) === 2 ? ' D' + depth : ''), .8, .8, -.7, c.height + 1, z);
        }
        ...
        const queues = {}, portQueues = [], routes = Model.routeConfigs(cfg), physical = new Map(), pointLabels = new Set();
        for (const route of routes) for (const kind of ['in', 'out']) {
          const key = Model.portKey(route.config, kind), p = M.port(route.config, kind);
          if (!physical.has(key)) physical.set(key, { key, p, kind, ids: [] });
          physical.get(key).ids.push(route.id);
        }
        for (const { key, p, kind, ids } of physical.values()) {
          const material = mat(kind === 'in' ? 0x2dc4bb : 0xf0a851);
          box(Math.min(1.5, px), .12, Math.max(loadD, .15), p.x, p.y - loadH / 2 - .16, z0 + p.z, material);
          this.label(kind.toUpperCase() + (routes.length > 1 ? ' ' + ids.join('/') : '') + ' (' + [p.x, p.y, p.z].map(v => Number(v.toFixed(2))).join(',') + ')', 4.2, .7, p.x, p.y + 1.1, z0 + p.z);
          this.labels.at(-1).center.y = kind === 'in' ? -.1 : 1.1;
          const group = new T.Group(); group.userData = { portKey: key, kind, coordinate: [p.x, p.y, p.z], routeIds: ids }; this.scene.add(group);
          if (!queues[kind]) queues[kind] = group;
          portQueues.push({ key, kind, group });
          for (let j = 0; j < 8; j++) box(.45, .2, .4, p.x - 1.4, p.y + .2 + j * .24, z0 + p.z, material, group);
        }
        this.aisles.push({ crane, carriage, fork, cargo, queues, portQueues, z: z0, lastVersion: -1, lastPose: '', engine: null });
        if (c.method === 'fem') for (const route of routes) {
          const f = Model.femPoints(route.config), points = [...f.inbound, ...(route.femCase === 2 ? f.outbound : [])];
          points.forEach((p, j) => {
            ...
            marker.userData = { point: p.name, cellId: p.id, aisle: i, route: route.id, femCase: route.femCase, coordinate: [p.x, p.y, p.z] }; this.scene.add(marker); this.pointMarkers.push(marker);
            const labelKey = [p.x, p.y, p.z, p.name].join(',');
            if (!pointLabels.has(labelKey)) { this.label(p.name, .9, .45, p.x, p.y + loadH * .75, z0 + p.z); pointLabels.add(labelKey); }
          });
        }
      }
      ...
      this.up = e => {
        if (e.button !== 0 || !this.pointerStart || Math.hypot(e.clientX - this.pointerStart.x, e.clientY - this.pointerStart.y) > 5) return;
    ...
    axisLabel(values, w, h, x, y, z, vertical, positions) {
      ...
      const fontSize = vertical ? Math.min(34, this.layout.pitch / h * plotHeight * .6) : Math.min(34, canvas.width / values.length * .45);
    ...
    setView(view, aisle = 0) {
      ...
      const size = Math.max(bounds.width / Math.min(1.4, aspect), bounds.height * 1.8, depth * 1.2, this.stride * 1.4, 10);
    ...
    update(fleet) {
      ...
      engines.forEach((e, i) => {
        ...
        const signature = [p.x.p, p.y.p, p.f.p, e.cargo, e.inSlots, e.outputCount, ...[...e.portBuffers.values()].map(b => b.count), effect, transfer?.stage].join(',');
        ...
        for (const port of a.portQueues) port.group.children.forEach((m, j) => { m.visible = j < (e.portBuffers.get(port.key)?.count || 0); });
      });
