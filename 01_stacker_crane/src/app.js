(function () {
  ...
  const groups = {
    ...
    flowFields: ['inbound', 'outbound', 'peak', 'cranes', 'targetUtil'], efficiencyFields: ['operatingEfficiency'], factorFields: ['A', 'Ft', 'Ew'],
  ...
  $('operatingEfficiency').placeholder = '예: 85';
  $('operatingEfficiency').setAttribute('aria-describedby', 'efficiencyHint efficiencyStatus');
  $('targetUtil').setAttribute('aria-describedby', 'targetUtilHint');
  $('targetUtil').insertAdjacentHTML('afterend', '<small id="targetUtilHint" class="speed-hint">필요 대수·부하 판정 기준입니다. 처리량 보정은 아래 운전효율에서 설정합니다.</small>');
  function refreshEfficiency() {
    const direct = $('operatingEfficiency').value.trim() !== '';
    for (const key of ['A', 'Ft', 'Ew', 'basis']) $(key).disabled = direct;
    $('efficiencyStatus').textContent = direct ? '직접 입력한 운전효율만 적용합니다. 세부 계수는 저장된 값을 유지하며 계산에는 사용하지 않습니다.' : '운전효율 미입력: 세부 계수와 근거가 있으면 A × Fₜ × E𝑤를 적용합니다.';
  }
  ...
  function write(input) {
    ...
    renderPortTable(input.ports); renderRoutes(input.routes || []); refreshMix(); refreshCaseHelp(); refreshUnits(); refreshRackHeights(); refreshDepth(); refreshEfficiency();
  }
  ...
  function apply() {
    try {
      ...
      $('factorIndicator').textContent = result.factor === null ? '미선택' : fmt(result.factor * 100, 1) + '%';
  ...
  function renderAnalysis() {
    ...
    $('analysisKpis').innerHTML = kpi('이론 처리량 · STC 1', fmt(r.theory), u, c.method === 'fem' ? 'FEM CASE ' + [...new Set(r.routes.map(a => a.femCase))].join('/') : '균일 셀 평균', true) +
      kpi('보정 처리량 · STC 1', fmt(r.available), u, Model.correctionText(r)) +
    ...
    const notes = [];
    if (r.factor === null) notes.push('이론 처리량을 계산했습니다. 왼쪽 「운전효율 및 보정」에 운전효율(예: 85)을 입력하면 보정 처리량과 필요 대수가 표시됩니다.');
    ...
    renderFem();
    $('sensitivityTable').innerHTML = r.factor === null ? '<p class="empty-state">운전효율 또는 보정계수를 입력하면<br>능력 변화와 필요 통로 수를 비교합니다.</p>' : C.table(['계수', '변화', '적용값', '능력 (' + u + ')', '필요 대수'], Model.sensitivity(r).map(row => [row.key === 'operatingEfficiency' ? '운전효율' : row.key, (row.delta > 0 ? '+' : '') + fmt(row.delta * 100, 0) + '%', fmt(row.value, 3) + (row.key === 'operatingEfficiency' ? '%' : ''), fmt(row.capacity), row.required]));
  }
  function renderFem() {
  ...
  $('operatingEfficiency').addEventListener('input', refreshEfficiency);
  ...
  $('assumedFactors').addEventListener('click', () => { $('operatingEfficiency').value = ''; $('A').value = .95; $('Ft').value = .95; $('Ew').value = 1; $('basis').value = 'assumption'; $('basisNote').value = '설계 검토 예시: A 0.95, Ft 0.95, Ew 1.0 (자동 운전 이상 조건). 실측값 아님.'; refreshEfficiency(); markDirty(); });
  $('idealFactors').addEventListener('click', () => { $('operatingEfficiency').value = ''; $('A').value = 1; $('Ft').value = 1; $('Ew').value = 1; $('basis').value = 'ideal'; $('basisNote').value = '고장·혼잡·작업효율 손실을 제외하는 이상 조건'; refreshEfficiency(); markDirty(); });
