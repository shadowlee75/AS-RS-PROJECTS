(function (root, factory) {
  ...
  class Engine {
    constructor(input, validated = false) {
      ...
      // Start each partially filled lane at the rear so no empty rear slot is trapped.
      if (Model.depthCount(c) === 2) for (let id = 0; id < this.rack.length; id += 2) {
        const front = this.rack[id], rear = this.rack[id + 1];
        if (front.state === 'occupied' && rear.state === 'empty') {
          rear.state = 'occupied'; rear.storedAt = front.storedAt; front.state = 'empty'; front.storedAt = 0;
        }
      }
    ...
    accessible(cell, state, incoming = null) {
      if (cell.state !== state) return false;
      if (Model.depthCount(this.config) === 1) return true;
      // A DC stores first: its inbound lane cannot also supply the outbound load.
      if (incoming && Model.sameLane(cell, incoming)) return false;
      if (cell.depth === 2) return this.rack[cell.id - 1].state === 'empty';
      return state !== 'empty' || this.rack[cell.id + 1].state === 'occupied';
    }
    chooseCell(state, rule, origin, random, incoming = null) {
      const eligible = this.rack.filter(cell => this.accessible(cell, state, incoming));
    ...
    dispatch() {
      ...
      let outCell = this.outQueue.length ? this.chooseCell('occupied', c.retrieval, inCell && c.mode === 'dc' ? inCell : this.position, this.retrieveRandom, c.mode === 'dc' ? inCell : null) : null;
      let kind;
    ...
    dispatchMixed() {
      const c = this.config, hasSpace = this.rack.some(p => this.accessible(p, 'empty')), hasStock = this.rack.some(p => this.accessible(p, 'occupied'));
      ...
      const cfg = this.routes[route].config;
      const inCell = inJob ? this.chooseCell('empty', c.storage, M.port(cfg, 'in'), this.storeRandom) : null;
      const outCell = outJob ? this.chooseCell('occupied', c.retrieval, inCell || this.position, this.retrieveRandom, inCell) : null;
      if (!inCell) inJob = null;
      if (!outCell) outJob = null;
      if (!inJob && !outJob) { this.state = 'idle'; return; }
      const kind = inJob && outJob ? 'dc' : inJob ? 'in' : 'out';
