(function (root, factory) {
  ...
  'use strict';
  const VERSION = '0.3.0', SCHEMA = 'stc/project@1';
  const defaults = {
  ...
  const enums = { method: ['cells', 'fem'], pointMode: ['cell', 'theory'], mode: ['sc-return', 'sc-stay', 'dc', 'mixed'], ratioBasis: ['cycles', 'loads'], storage: ['random', 'nearest'], retrieval: ['fifo', 'random', 'nearest'],
  ...
  function calculate(input) {
    ...
    for (let i = 0; i < c.cranes; i++) {
      ...
      if (routes.length > 1 || c.mode === 'mixed') {
        ...
        for (const key of ['scIn', 'scOut', 'sc', 'dc', 'returnTime']) combined[key] = key === 'dc' && n < 2 ? null : average(key);
  ...
  return { VERSION, SCHEMA, defaults, fields, enums, nullable, portKeys, migrate, aisleConfig, routeConfigs, portKey, mix, address, caseNames, caseConditions, casePorts, femPortMatch, femReference, femPoints, nearestCell, rackLayout, envelope, validate, rng, cells, calculate, sensitivity, pack, unpack };
});
