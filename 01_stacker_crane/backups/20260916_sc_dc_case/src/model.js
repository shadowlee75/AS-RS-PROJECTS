(function (root, factory) {
  ...
  'use strict';
  const VERSION = '0.2.6', SCHEMA = 'stc/project@1';
  const defaults = {
    name: '스태커크레인 설계 검토', length: 60, height: 20, bays: 30, levels: 10, sides: 2, firstLevelHeight: null, levelPitch: null,
    ...
    mode: 'dc', storage: 'random', retrieval: 'fifo', arrival: 'takt', jitter: 0.1,
  ...
  const fields = {
    ...
    bays: ['Bay 수', '개', 1, 80, 1], levels: ['Level 수', '단', 1, 30, 1],
    ...
    stroke: ['포크 편도 스트로크', 'm', 0.001, 10, 0.1], vf: ['포크 최대속도', 'speed', 0.001, 5, 0.1],
  ...
  const enums = { method: ['cells', 'fem'], pointMode: ['cell', 'theory'], mode: ['sc-return', 'sc-stay', 'dc'], storage: ['random', 'nearest'], retrieval: ['fifo', 'random', 'nearest'],
  ...
  const nullable = ['A', 'Ft', 'Ew', 'ioZ', 'outX', 'outY', 'outZ', 'firstLevelHeight', 'levelPitch'];
  ...
  function validate(input) {
    ...
    for (let i = 0; i < c.cranes; i++) {
      ...
      for (const kind of ['in', 'out']) {
        const p = M.port(local, kind);
        if (Math.abs(p.z) > c.stroke)
          throw Error('STC ' + (i + 1) + ' ' + kind.toUpperCase() + ': |Z|는 포크 스트로크 이하여야 합니다. X·Y는 랙 외부 좌표를 허용합니다.');
      }
      if (c.method === 'fem') femPoints(local);
    }
    if (c.bays * c.levels * c.sides * c.cranes > 120000) throw Error('전체 랙 셀은 120,000개 이내로 입력하세요. 모든 STC와 셀을 함께 재현하기 위한 한도입니다.');
  ...
  function aisleConfig(c, index) {
    const local = { ...c, ports: [] }, overrides = c.ports?.[index] || {};
    for (const key of portKeys) if (overrides[key] != null) local[key] = overrides[key];
    return local;
  }
  function address(cell, aisle = 0) {
    return 'STC' + String(aisle + 1).padStart(2, '0') + '-' + (cell.side < 0 ? 'A' : 'B') + '-' + String(cell.bay).padStart(3, '0') + '-' + String(cell.level).padStart(2, '0');
  }
  ...
  function femPoints(c) {
    ...
    const snapPair = points => {
      let previous = null;
      return points.map(p => {
        const cell = nearestCell(rack, p, rack.length > 1 ? previous : null); previous = cell.id;
  ...
  function cells(c) {
    const layout = rackLayout(c);
    const result = [];
    for (let side = 0; side < c.sides; side++) for (let b = 0; b < c.bays; b++) for (let l = 0; l < c.levels; l++) {
      result.push({ id: result.length, bay: b + 1, level: l + 1, side: side === 0 ? -1 : 1,
        x: (b + 0.5) * c.length / c.bays, y: layout.heights[l], z: (side === 0 ? -1 : 1) * c.stroke });
    }
    return result;
  }
  function envelope(c) {
    const layout = rackLayout(c), points = [];
    for (let i = 0; i < c.cranes; i++) { const a = aisleConfig(c, i); points.push(M.port(a, 'in'), M.port(a, 'out')); }
  ...
  function calculate(input) {
    ...
    if (c.method === 'cells' && n > 1) {
      if (exact) { for (let i = 0; i < n; i++) for (let j = 0; j < n; j++) if (i !== j) add(i, j); }
      else { const random = rng(9851); for (let k = 0; k < 30000; k++) {
        const i = Math.floor(random() * n); let j = Math.floor(random() * (n - 1)); if (j >= i) j++; add(i, j);
      } }
    }
    const total = c.inbound + c.outbound;
    const p = c.mode === 'dc' && n > 1 ? (total > 0 ? Math.min(c.inbound, c.outbound) / Math.max(c.inbound, c.outbound) : 1) : 0;
    ...
    function one(cfg) {
      ...
      const inPoints = fem ? fem.inbound : rack, outPoints = fem ? fem.outbound : rack;
      ...
      let dc = null, dcPlans = [];
      if (n > 1) {
        if (fem) { dcPlans = fem.dcPairs.map(pair => M.plan(cfg, 'dc', E, pair[0], pair[1], true)); dc = mean(dcPlans); }
        else {
          ...
          dc = loadedIn + between + loadedOut + travelTime(cfg, A, E, false)
            + M.fork(cfg, Math.abs(E.z)).duration + M.fork(cfg, Math.abs(A.z)).duration + 2 * M.fork(cfg).duration + cfg.control;
        }
      }
    ...
    const aisles = [];
    for (let i = 0; i < c.cranes; i++) {
      const cfg = aisleConfig(c, i), key = JSON.stringify([M.port(cfg, 'in'), M.port(cfg, 'out')]);
      if (!cache.has(key)) cache.set(key, one(cfg));
      aisles.push({ ...cache.get(key), aisle: i + 1 });
    }
    ...
    return { ...first, config: c, p, demand, factor, capacity, util, worstUtil, required, theoryTotal, aisles,
      cells: n, totalCells: n * c.cranes, fork: M.fork(c).duration,
      dcSampling: { method: c.method === 'fem' ? 'FEM 대표 경로' : exact ? '전수 열거' : '고정 시드 표본',
        samples: c.method === 'fem' ? first.fem.dcPairs.length : samples, se: !exact && samples > 1 ? Math.sqrt(m2 / (samples - 1) / samples) : 0 },
      reference: (c.method === 'fem' ? 'FEM 9.851 (06.2003) CASE ' + c.femCase + (c.pointMode === 'cell' ? ' 기준 실제 화물 중심 · 설계 계산' : ' 이론 대표점 비교') + (aisles.some(a => !a.fem.portMatch) ? ' · 실제 외부/자유 포트 적용' : '') : '균일 셀 선택 · SC-return/DC 공학 모델')
  ...
  function unpack(data) {
    ...
    if (typeof data.moduleVersion !== 'string' || !['0.1', '0.2'].includes(data.moduleVersion.split('.').slice(0, 2).join('.')))
      throw Error('프로그램 주·부 버전이 다릅니다. 버전 변환 후 불러오세요.');
    return validate(data.config);
  }
  return { VERSION, SCHEMA, defaults, fields, enums, nullable, portKeys, migrate, aisleConfig, address, caseNames, caseConditions, casePorts, femPortMatch, femReference, femPoints, nearestCell, rackLayout, envelope, validate, rng, cells, calculate, sensitivity, pack, unpack };
});
