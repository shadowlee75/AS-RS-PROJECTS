(function (root) {
  ...
  const labels = { method: { cells: '전체 셀 평균', fem: 'FEM CASE 기준' }, pointMode: { cell: '실제 화물 중심', theory: '이론 좌표 비교' },
    mode: { 'sc-return': 'SC - I/O 복귀', 'sc-stay': 'SC - 제자리 대기', dc: 'DC - 입출고 페어링' },
  ...
  function documentHtml(result, snapshot, options = {}) {
    ...
    const inputRows = keys => keys.map(key => {
      ...
      const value = labels[key] ? labels[key][raw] : raw === null ? (['firstLevelHeight', 'levelPitch'].includes(key) ? '자동' : '미입력') : key === 'transferLift' ? n(raw * 1000, 0) :
        typeof raw === 'number' ? n(raw * (isSpeed && c.speedUnit === 'm/min' ? 60 : 1), Number.isInteger(raw) && !isSpeed ? 0 : 3) : raw;
      return [field?.[0] || ({ mode: '운전 모드', storage: '입고 셀 선택', retrieval: '출고 셀 선택', arrival: '작업 도착', basis: '보정 근거', method: '계산 방식', pointMode: 'P1/P2 위치' })[key] || key,
        value, isSpeed ? c.speedUnit : field?.[1] === 'unit/h' ? unit : field?.[1] === 'ratio' ? '비율' : field?.[1] || '-'];
    });
    const judgement = result.factor === null ? '판정 보류 - 보정계수와 근거를 입력해야 설계 능력을 산정할 수 있습니다.' :
    ...
    section('1. 설비 및 동작 사양',
      '<h3>랙 구성</h3>' + table(['항목', '값', '단위'], inputRows(['length', 'height', 'bays', 'levels', 'sides', 'firstLevelHeight', 'levelPitch', 'cranes'])) +
      ...
      table(['이재 및 지연 항목', '값', '단위'], inputRows(['stroke', 'transferLift', 'forkDwell', 'position', 'control', 'loadedFactor'])) +
    ...
    section('2. 운영 조건 및 계산 기준',
      '<h3>요구량 및 설계 보정</h3>' + table(['항목', '값', '단위'], inputRows(['inbound', 'outbound', 'peak', 'targetUtil', 'A', 'Ft', 'Ew', 'basis'])) +
    ...
    portGroups.forEach((aisles, i) => section('3. STC별 포트와 계산 결과' + (portGroups.length > 1 ? ' (' + (i + 1) + '/' + portGroups.length + ')' : ''),
      ...
      table(['STC', 'IN X / Y / Z (m)', 'OUT X / Y / Z (m)'], aisles.map(a => [a.aisle, xyz(a.ports.in), xyz(a.ports.out)])) +
    ...
    if (result.fem) {
      const rows = result.aisles.flatMap(a => [...a.fem.inbound, ...(c.femCase === 2 ? a.fem.outbound : [])].map(p => [a.aisle, p.name,
      ...
      groups.forEach((rows, i) => section('4. FEM 대표점 적용' + (groups.length > 1 ? ' (' + (i + 1) + '/' + groups.length + ')' : ''),
        note('FEM 9.851 (06.2003) §4.' + c.femCase + ', CASE ' + c.femCase + ' / ' + Model.caseNames[c.femCase - 1] + '. 참조 조건: ' + Model.caseConditions[c.femCase - 1]) +
        ...
        note(result.aisles.some(a => !a.fem.portMatch) ? '실제 포트가 CASE 참조 배치와 다른 설계 확장입니다. CASE 기준으로 셀을 선정하고 실제 외부/자유 포트까지의 이동시간을 계산했습니다.' : 'CASE 참조 포트 조건과 일치합니다.') +
        note('SC는 대표점별 복귀 사이클 평균, DC는 IN → P1 → P2 → OUT → IN입니다. CASE 2는 기본·대칭 경로를 평균합니다.')));
      const extended = result.aisles.filter(a => !a.fem.portMatch);
      chunk(extended, 16).forEach((aisles, i) => section('FEM 셀 선정 참조 포트' + (i ? ' (계속)' : ''),
        table(['STC', '참조 IN XYZ (m)', '참조 OUT XYZ (m)', '좌우 반전'], aisles.map(a => [a.aisle, xyz(a.fem.referencePorts.in), xyz(a.fem.referencePorts.out), a.fem.mirroredLayout ? '적용' : '없음'])) +
    ...
    section('적용 조건 및 민감도',
      ...
      note('보정 능력=이론 능력×A×Ft×Ew. 필요 대수=ceil(피크 총 요구량/(현재 STC 중 최저 보정 능력×목표 부하율)). 독립 통로에 부하를 균등 배분하며 공통 상하류 병목은 별도 검토합니다.') +
      ...
      note('Single Deep / Single Fork, 균등 랙, 통로당 1대입니다. creep, jerk 제한 S-curve, 고장, 다중 깊이, Twin Fork, 공유레일 충돌은 포함하지 않습니다. 현장 실측 및 규격 적합성 판정은 별도입니다.') +
      '<h3>보정계수 민감도</h3>' +
      (result.factor === null ? note('보정계수와 근거 미입력으로 민감도를 산정하지 않았습니다.') : table(['계수', '변경률', '적용값', '전체 보정 능력 (' + unit + ')', '필요 대수'],
        Model.sensitivity(result).map(r => [r.key, n(r.delta * 100, 0) + '%', n(r.value), n(r.capacity, 2), r.required]))) +
      note('입력 A·Ft·Ew 중 1.0은 해당 손실을 제외하는 조건입니다. 이론 능력, 종료 대기 작업, 재고 부족·만재 및 버퍼 차단을 함께 검토합니다.'));
