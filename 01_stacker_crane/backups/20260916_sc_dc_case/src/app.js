(function () {
  ...
  const modeNames = { 'sc-return': 'SC · I/O 복귀', 'sc-stay': 'SC · 제자리 대기', dc: 'DC · 입출고 페어링' };
  ...
  function write(input) {
    ...
    renderPortTable(input.ports); refreshCaseHelp(); refreshUnits(); refreshRackHeights();
  }
  ...
  function readPortTable() {
    return [...$('portTable').querySelectorAll('tbody tr')].map(row => Object.fromEntries(Model.portKeys.map(key => {
  ...
  function renderPortTable(rows = []) {
    ...
    $('portTable').innerHTML = '<table><thead><tr><th>STC</th>' + ['IN X', 'IN Y', 'IN Z', 'OUT X', 'OUT Y', 'OUT Z'].map(k => '<th>' + k + '</th>').join('') + '</tr></thead><tbody>' +
      Array.from({ length: n }, (_, i) => '<tr><th>' + (i + 1) + '</th>' + Model.portKeys.map(key => '<td><input type="number" step="0.1" data-port="' + key + '" aria-label="STC ' + (i + 1) + ' ' + key + '" placeholder="기본" value="' + esc(rows[i]?.[key] ?? '') + '"></td>').join('') + '</tr>').join('') + '</tbody></table>';
  ...
  function motionConfig() { return Model.aisleConfig(config, Number($('motionAisle').value) || 0); }
  ...
  function apply() {
    try {
      ...
      buildPreview(); renderAnalysis(); resetEngine();
  ...
  function renderAnalysis() {
    ...
    $('analysisKpis').innerHTML = kpi('이론 처리량 · STC 1', fmt(r.theory), u, c.method === 'fem' ? 'FEM CASE ' + c.femCase : '균일 셀 평균', true) +
  ...
  function renderFem() {
    const r = result, c = config;
    const extended = r.fem && r.aisles.some(a => !a.fem.portMatch);
    $('femSummary').innerHTML = '<p>' + esc(r.reference) + '</p>' +
      (r.fem ? '<div class="notice ' + (extended || r.fem.alpha <= .5 || r.fem.alpha >= 2 ? 'warning' : 'subtle') + '">CASE ' + c.femCase + ' · 참조 조건 ' + esc(r.fem.condition) + ' · α = ' + fmt(r.fem.alpha, 3) + (r.fem.alpha <= .5 || r.fem.alpha >= 2 ? ' · 권고 범위 0.5 < α < 2 밖' : '') + '</div><p>' + (c.pointMode === 'cell' ? 'P1·P2는 실제 화물 중심입니다. 아래 적용 XYZ와 셀 주소를 계산·배치도·3D·운동 그래프에서 공통 사용합니다. 이론 XYZ는 셀 선정의 비교 기준입니다.' : '이론 좌표 비교 모드입니다. 계산과 그래프에 규정의 연속 좌표를 사용합니다.') + '</p>' + (extended ? '<p class="notice subtle">실제 포트가 CASE 참조 배치와 달라, CASE 기준 셀 선정에 실제 포트 이동거리를 결합한 설계 계산입니다. 원 규정 배치의 이론 계산과 구분합니다.</p>' : '') + '<p class="muted">SC는 대표점별 완전 복귀 사이클의 평균, DC는 IN → P1 → P2 → OUT → IN입니다. CASE 2는 기본·대칭 경로의 평균입니다. 두 대표점이 같은 셀에 가까우면 P2는 다른 셀 중 가장 가까운 중심을 선택합니다.</p>' : '<p class="muted">현재는 모든 실제 화물 중심의 평균입니다. 왼쪽에서 CASE 기준 대표 셀 계산을 선택할 수 있습니다. 외부 포트 좌표는 그대로 사용합니다.</p>');
    const rows = [];
    if (r.fem) r.aisles.forEach((a, i) => {
      const rack = Model.cells(c), points = [...a.fem.inbound, ...(c.femCase === 2 ? a.fem.outbound : [])];
      points.forEach(p => { const nearest = p.id == null ? Model.nearestCell(rack, p) : p, ref = p.reference || p;
        rows.push([i + 1, p.name, fmt(p.x, 3), fmt(p.y, 3), fmt(p.z, 3), Model.address(nearest, i), [ref.x, ref.y, ref.z].map(v => fmt(v, 3)).join(', ')]); });
    });
    $('femPoints').innerHTML = rows.length ? C.table(['STC', '대표점', '적용 X (m)', '적용 Y (m)', '적용 Z (m)', c.pointMode === 'cell' ? '적용 화물 셀' : '가까운 셀 (참고)', '이론 XYZ (비교)'], rows) : '';
    $('referencePorts').innerHTML = extended ? C.table(['STC', '셀 선정 참조 IN XYZ', '셀 선정 참조 OUT XYZ', '좌우 반전'], r.aisles.map(a => [a.aisle, [a.fem.referencePorts.in.x, a.fem.referencePorts.in.y, a.fem.referencePorts.in.z].map(v => fmt(v, 3)).join(', '), [a.fem.referencePorts.out.x, a.fem.referencePorts.out.y, a.fem.referencePorts.out.z].map(v => fmt(v, 3)).join(', '), a.fem.mirroredLayout ? '적용' : '없음'])) : '';
    $('aisleAnalysis').innerHTML = C.table(['STC', 'IN XYZ (m)', 'OUT XYZ (m)', 'SC 입고 s', 'SC 출고 s', 'DC s', 'OUT→IN s', '이론 /h'], r.aisles.map(a => [a.aisle,
      [a.ports.in.x, a.ports.in.y, a.ports.in.z].map(v => fmt(v, 2)).join(', '), [a.ports.out.x, a.ports.out.y, a.ports.out.z].map(v => fmt(v, 2)).join(', '), fmt(a.scIn, 2), fmt(a.scOut, 2), fmt(a.dc, 2), fmt(a.returnTime, 2), fmt(a.theory, 2)]));
  }
  function buildPreview() {
    ...
    $('previewDescription').textContent = '입고 ' + label(previewIn) + ' · 출고 ' + label(previewOut) + ' · ' + (source === 'fem' && c.pointMode === 'theory' ? 'FEM 이론 좌표 비교' : '실제 화물 중심 경로') + ' · DC는 OUT→IN 공차 복귀 포함.';
  ...
  function renderSimulation() {
    ...
    $('fleetTable').innerHTML = C.table(['STC', '동작', '현재 / 목표 셀', 'X / Y / Z (m)', '입고', '출고', '재고', '대기', '처리량 /h'], engine.engines.map((e, i) => {
      ...
      return [i + 1, p.label, target.bay ? Model.address(target, i) : target.port ? target.port.toUpperCase() + ' 포트' : '—', [p.x.p, p.y.p, p.f.p].map(v => fmt(v, 2)).join(' / '), local.completed.in, local.completed.out, local.stock, local.queued, fmt(local.throughput)];
    }));
  ...
  function renderExperiments() {
    const rows = [];
    for (const mode of ['sc-return', 'sc-stay', 'dc']) {
  ...
  async function experiments() {
    ...
    try {
      for (let r = 0; r < n; r++) for (const mode of ['sc-return', 'sc-stay', 'dc']) {
        ...
        experimentData.runs.push({ mode, repetition: r, seed: runConfig.seed, throughput: s.throughput, p95Wait: s.p95Wait, queued: s.queued, busy: s.busy,
          stock: s.stock, inProgress: s.inProgress, overflowTime: s.overflowTime, blockedTime: s.stateTime.blocked, conservation: s.conservation, jobsConserved: s.jobsConserved });
        $('experimentProgress').value = experimentData.runs.length / (n * 3) * 100;
        $('experimentStatus').textContent = experimentData.runs.length + ' / ' + n * 3 + '회 완료 · ' + modeNames[mode] + ' · seed ' + runConfig.seed;
        $('exportExperiments').disabled = false; renderExperiments();
      }
      $('experimentStatus').textContent = n * 3 + '회 완료 · Student t 95% 신뢰구간 · 처리량 단위 ' + config.unit + '/h · 각 반복의 P95 대기 평균';
  ...
  $('applyCase').addEventListener('click', () => {
    ...
    renderPortTable(readPortTable().map(row => ({ ...row, ioX: null, ioY: null, outX: null, outY: null, ...([1, 3, 4].includes(c.femCase) ? { outZ: row.ioZ } : {}) })));
  ...
  for (const id of ['motionAisle', 'motionSource', 'femRoute']) $(id).addEventListener('change', () => { if (fresh()) { try { buildPreview(); } catch (e) { toast(e.message); } } });
  ...
  $('projectFile').addEventListener('change', async () => {
    ...
    try { if (file.size > 1000000) throw Error('프로젝트 JSON은 1MB 이하여야 합니다.'); const c = Model.unpack(JSON.parse(await file.text())); write(c); if (apply()) toast('프로젝트 설정을 불러왔습니다.'); }
  ...
  $('exportSim').addEventListener('click', () => { if (!fresh()) return; const rows = [['job_id', 'STC', 'rack_address', 'port_x_m', 'port_y_m', 'port_z_m', 'kind', 'arrival_s', 'dispatch_s', 'complete_s', 'wait_s', 'lead_s', 'cycle', 'measurement_window', 'seed']]; engine.logs.forEach(job => rows.push([job.id, job.aisle, job.address, job.port.x, job.port.y, job.port.z, job.kind, job.arrival, job.start, job.end, job.wait, job.lead, job.cycle, job.measured, job.seed])); download(fileName('_완료작업.csv'), R.csv(rows), 'text/csv;charset=utf-8'); });
