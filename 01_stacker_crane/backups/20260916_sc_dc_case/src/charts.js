(function (root) {
  ...
  function rack(c, a, b, engine) {
    const Model = root.STCModel, input = M.port(c, 'in'), output = M.port(c, 'out'), layout = Model.rackLayout(c), bounds = Model.envelope(c);
    ...
    for (const cell of (engine ? engine.rack : Model.cells(c)).filter(p => p.side === -1)) {
      ...
      s += '<rect data-cell="' + cell.id + '" data-x="' + cell.x + '" data-y="' + cell.y + '" x="' + (sx(cell.x) - cellW * .3) + '" y="' + (sy(cell.y) - cellH * .27) + '" width="' + cellW * .6 + '" height="' + cellH * .54 + '" rx="1" fill="' + color + '" opacity=".7"><title>' + cell.bay + '번지 ' + cell.level + '단 · 중심 (' + fmt(cell.x, 3) + ', ' + fmt(cell.y, 3) + ') m</title></rect>';
    }
    if (engine) {
    ...
    for (const [p, key, label, color, dy] of [[input, 'in', 'E / IN', colors.x, -14], [output, 'out', 'A / OUT', colors.y, 18]])
      s += '<circle data-port="' + key + '" cx="' + sx(p.x) + '" cy="' + sy(p.y) + '" r="6" fill="' + color + '" stroke="white" stroke-width="2"/><text x="' + sx(p.x) + '" y="' + (sy(p.y) + dy) + '" text-anchor="middle" fill="' + color + '" font-size="10">' + label + '<title>XYZ (' + [p.x, p.y, p.z].map(v => fmt(v, 3)).join(', ') + ') m</title></text>';
    s += '<text x="375" y="278" text-anchor="middle" fill="#8ba0ad" font-size="10">랙 ' + fmt(c.length) + ' m · ' + c.bays + '번지 × ' + c.levels + '단 · 포트 포함 X ' + fmt(bounds.xMin) + '~' + fmt(bounds.xMax) + ' m</text></svg>';
    return s;
  }
