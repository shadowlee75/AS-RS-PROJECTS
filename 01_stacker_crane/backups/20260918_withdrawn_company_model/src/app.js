(function () {
  ...
  const speedKeys = Object.keys(Model.fields).filter(key => Model.fields[key][1] === 'speed');
  ...
  function inputScale(key) { return key === 'transferLift' ? 1000 : speedKeys.includes(key) && speedUnit === 'm/min' ? 60 : 1; }
  function field(key) {
  ...
  const groups = {
    ...
    forkFields: ['stroke', 'rearStroke', 'vf', 'af', 'df', 'transferLift', 'forkDwell'], delayFields: ['position', 'control', 'loadedFactor'],
    creepFields: ['lowVx', 'lowVy', 'lowVf', 'approachX', 'approachY', 'approachF', 'moveAllowance'],
  ...
  function refreshMotionModel() {
    const creep = $('motionModel').value === 'creep';
    $('creepSettings').hidden = !creep;
    $('transferHint').textContent = creep ? '저속 접근 모드: 캐리지 이재시간 = 승하강량 ÷ 승강 저속. 포크 이재 1회 = 신장 + 저속 승강 + 안착 대기 + 수축입니다. 회사의 포크 여유시간은 안착 대기에 넣고, 이동 여유시간과 별도 정위치 시간을 중복 입력하지 마세요.' : '상차: 포크 OUT → 캐리지 UP → 포크 IN. 하차: 포크 OUT → 캐리지 DOWN → 포크 IN. 승하강량 기본 110 mm. 승강 Y 속도·가감속과 적재 비율로 시간을 계산하고, 안착 대기는 승하강 완료 후 적용합니다.';
  }
  function refreshDepth() {
    const double = $('rackDepth').value === 'double';
    $('rearStroke').disabled = !double; $('rearStroke').closest('.field').hidden = !double;
    $('rearStroke').placeholder = '자동 · 앞열의 2배';
    $('depthHint').textContent = double ? '더블딥: 각 번지·단·면에 앞열(D1)과 뒷열(D2)을 생성합니다. 뒷열 스트로크는 통로 중심부터의 총 거리이며, 빈칸은 앞열의 2배를 적용합니다. SC/DC 운전 모드와 별개입니다.' : '싱글딥: 각 번지·단·면에 화물 1개를 보관합니다. 보관면 수와 SC/DC 운전 모드는 별도로 선택합니다.';
  }
  ...
  function write(input) {
    ...
    renderPortTable(input.ports); renderRoutes(input.routes || []); refreshMix(); refreshCaseHelp(); refreshUnits(); refreshRackHeights(); refreshDepth(); refreshMotionModel();
  }
  function read(check = true) {
    ...
    for (const key of Object.keys(Model.defaults)) {
      ...
      if (speedKeys.includes(key) && displayUnit === 'm/min') c[key] /= 60;
  ...
  function showCell(id, aisle) {
    ...
    $('cellBay').value = cell.bay; $('cellLevel').value = cell.level; $('cellSide').value = cell.side; $('cellDepth').value = cell.depth || 1;
  ...
  function apply() {
    try {
      ...
      $('projectSubtitle').textContent = config.name + ' · ' + Model.depthName(config) + ' · ' + config.bays + ' Bay × ' + config.levels + ' Level · ' + modeNames[config.mode];
      $('depthBadge').textContent = (Model.depthCount(config) === 2 ? 'Double Deep' : 'Single Deep') + ' · Single Fork';
      ...
      $('previewKind').querySelector('[value="dc"]').disabled = result.dc === null;
      if (result.dc === null) $('previewKind').value = 'in';
      for (const id of ['cellDepth', 'previewDepth', 'previewOutDepth']) {
        $(id).querySelector('[value="2"]').disabled = Model.depthCount(config) === 1;
        $(id).disabled = Model.depthCount(config) === 1;
        if ($(id).disabled) $(id).value = '1';
      }
  ...
  function renderAnalysis() {
    ...
    if (Model.depthCount(c) === 2) notes.push(Model.depthNote);
    if (c.motionModel === 'creep') notes.push('저속 접근과 이동 여유시간을 반영합니다. 이재 승강은 승강 저속을 사용합니다.');
    if (c.pointMode === 'continuous') notes.push('연속 대표점으로 계산합니다. 3D 셀 배치는 균등 배치 개략도이며 실제 주소와 다를 수 있습니다.');
    ...
    $('rackCount').textContent = Model.depthName(c) + ' · ' + r.cells.toLocaleString() + '셀/STC × ' + c.cranes + '대 = ' + r.totalCells.toLocaleString() + '셀';
    ...
    if (r.factor !== null) $('cycleSummary').innerHTML += metric('SC / DC 보정 물동량', fmt(3600 / r.sc * r.factor, 1) + ' / ' + fmt(r.dc ? 7200 / r.dc * r.factor : null, 1) + ' <small>' + u + '</small>', 'STC 1 · SC 100% / DC 100% 각각의 참고 능력');
  ...
  function buildPreview() {
    ...
    if (source === 'fem') {
      ...
      const cell = (bay, level, side, depth) => rack.find(p => p.bay === Number($(bay).value) && p.level === Number($(level).value) && p.side === Number($(side).value) && (p.depth || 1) === Number($(depth).value));
      previewIn = cell('previewBay', 'previewLevel', 'previewSide', 'previewDepth'); previewOut = cell('previewOutBay', 'previewOutLevel', 'previewOutSide', 'previewOutDepth');
      ...
      if (kind === 'dc' && Model.depthCount(c) === 2 && Model.sameLane(previewIn, previewOut)) throw Error('더블딥 DC 입고·출고는 서로 다른 번지·단·면을 선택하세요. 같은 위치의 앞열·뒷열은 접근이 차단됩니다.');
    }
    if (source === 'fem' && Model.depthCount(c) === 2) {
      const atDepth = (point, depth) => point.id == null ? { ...point, depth, z: point.side * Model.depthStroke(c, depth) } : { ...point, ...rack.find(cell => Model.sameLane(cell, point) && cell.depth === depth) };
      previewIn = atDepth(previewIn, Number($('previewDepth').value)); previewOut = atDepth(previewOut, Number($('previewOutDepth').value));
    }
    ...
    $('previewDescription').textContent = $('motionRoute').selectedOptions[0].textContent + ' · 입고 ' + label(previewIn) + ' · 출고 ' + label(previewOut) + ' · ' + (source === 'fem' && c.pointMode !== 'cell' ? '연속 대표점 경로' : '실제 화물 중심 경로') + ' · DC는 OUT→IN 공차 복귀 포함.';
    ...
    $('phaseTable').innerHTML = C.table(['축', '거리 (m)', '선도', '도달속도 (' + config.speedUnit + ')', '가속 (s)', '정속 (s)', '감속 (s)', '합계 (s)'],
      [['주행 X', move.x], ['승강 Y', move.y], ['입고 셀 포크 Z', M.fork(c, Math.abs(previewIn.z)).profile], ['출고 셀 포크 Z', M.fork(c, Math.abs(previewOut.z)).profile], ['IN 포크 Z', M.fork(c, Math.abs(M.port(c, 'in').z)).profile], ['OUT 포크 Z', M.fork(c, Math.abs(M.port(c, 'out').z)).profile], ['캐리지 이재 UP / DOWN', M.fork(c).lift]]
  ...
  $('motionModel').addEventListener('change', refreshMotionModel);
  $('configForm').addEventListener('input', event => { if (event.target.id !== 'speedUnit') { markDirty(); if (speedKeys.includes(event.target.id)) refreshSpeedHints(); if (['height', 'levels', 'firstLevelHeight', 'levelPitch'].includes(event.target.id)) refreshRackHeights(); } });
  ...
  $('speedUnit').addEventListener('change', () => {
    ...
    if (next !== speedUnit) for (const key of speedKeys) if ($(key).value.trim() !== '') $(key).value = Number((Number($(key).value) * factor).toPrecision(12));
  ...
  $('rackDepth').addEventListener('change', refreshDepth);
  ...
  $('findCell').addEventListener('click', () => { if (!fresh()) return; const i = Number($('simAisle').value), cell = engine.engines[i].rack.find(c => c.bay === Number($('cellBay').value) && c.level === Number($('cellLevel').value) && c.side === Number($('cellSide').value) && (c.depth || 1) === Number($('cellDepth').value)); if (!cell) { toast('번지·단·면 입력이 랙 범위를 벗어났습니다.'); return; } ensureScene(); showCell(cell.id, i); });
  ...
  $('loadCompany').addEventListener('click', () => { $('companyFile').value = ''; $('companyFile').click(); });
  $('companyFile').addEventListener('change', async () => {
    const file = $('companyFile').files[0]; if (!file) return;
    try {
      if (file.size > 1000000) throw Error('회사 입력 파일은 1MB 이하여야 합니다.');
      const bytes = await file.arrayBuffer(); let text;
      try { text = new TextDecoder('utf-8', { fatal: true }).decode(bytes); } catch (_) { text = new TextDecoder('euc-kr').decode(bytes); }
      const c = STCCompany.parse(text, config);
      document.activeElement?.blur(); write(c);
      if (apply()) toast('회사 입력을 적용했습니다. 요구량은 기존 설정을 유지하며 SC/DC 보정 물동량을 함께 표시합니다.');
    } catch (error) { toast('회사 입력 불러오기 실패: ' + error.message); }
  });
