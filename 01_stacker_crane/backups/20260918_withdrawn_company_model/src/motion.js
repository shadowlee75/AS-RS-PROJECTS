(function (root, factory) {
  ...
  function constant(distance, speed, label = '저속') {
    const duration = distance / speed;
    return { distance, speed, accel: 0, decel: 0, peak: distance ? speed : 0, critical: 0, duration, ta: 0, tc: duration, td: 0,
      shape: distance ? label : '정지', phases: duration ? [{ label, start: 0, duration, position: 0, velocity: speed, acceleration: 0 }] : [] };
  }
  // Company-compatible approach model: accelerate, cruise, decelerate to low
  // speed, traverse the minimum approach distance, then allow for positioning.
  // The final low-speed stop is instantaneous in this legacy time model.
  function creepAxis(distance, speed, accel, decel, low, approach, allowance = 0) {
    if (![distance, speed, accel, decel, low, approach, allowance].every(Number.isFinite) || distance < 0 || speed <= 0 || accel <= 0 || decel <= 0 || low <= 0 || low > speed || approach < 0 || allowance < 0)
      throw Error('저속 접근: 0 < 저속 ≤ 최대속도, 가감속 > 0, 거리·여유시간 ≥ 0을 입력하세요.');
    if (distance <= approach || distance === 0) return constant(distance, low, '저속 접근');
    const critical = speed * speed / (2 * accel) + (speed * speed - low * low) / (2 * decel) + approach;
    const peak = Math.min(speed, Math.sqrt(2 * (distance - approach + low * low / (2 * decel)) / (1 / accel + 1 / decel)));
    const ta = peak / accel, td = (peak - low) / decel;
    const tc = Math.max(0, distance - critical) / speed;
    const duration = ta + td + tc + approach / low + allowance;
    // Below the distance required to attain low speed the legacy formula is
    // a time estimate, not a feasible trapezoid. Keep its time but interpolate
    // this exceptional tiny segment monotonically rather than backwards.
    if (td < 0) {
      const p = constant(distance, distance / (duration - allowance), '미소거리 시간 환산');
      if (allowance) p.phases.push({ label: '이동 여유', start: p.duration, duration: allowance, position: distance, velocity: 0, acceleration: 0 });
      return { ...p, duration, critical, allowance, approximate: true };
    }
    const phases = []; let t = 0, p = 0, v = 0;
    function phase(label, dt, a) {
      if (dt <= 0) return;
      phases.push({ label, start: t, duration: dt, position: p, velocity: v, acceleration: a });
      p += v * dt + a * dt * dt / 2; v += a * dt; t += dt;
    }
    phase('가속', ta, accel); phase('정속', tc, 0); phase('저속까지 감속', td, -decel);
    v = low; phase('저속 접근', approach / low, 0);
    p = distance; v = 0; phase('이동 여유', allowance, 0);
    return { distance, speed, accel, decel, low, approach, allowance, peak, critical, duration: t, ta, tc, td,
      shape: tc > 1e-9 ? '사다리꼴 + 저속' : '삼각형 + 저속', phases };
  }
  ...
  function move(c, from, to, loaded) {
    ...
    const creep = c.motionModel === 'creep';
    const x = creep ? creepAxis(Math.abs(to.x - from.x), c.vx * f, c.ax * f, c.dx * f, c.lowVx * f, c.approachX, c.moveAllowance) : axis(Math.abs(to.x - from.x), c.vx * f, c.ax * f, c.dx * f);
    const y = creep ? creepAxis(Math.abs(to.y - from.y), c.vy * f, c.ay * f, c.dy * f, c.lowVy * f, c.approachY, c.moveAllowance) : axis(Math.abs(to.y - from.y), c.vy * f, c.ay * f, c.dy * f);
  ...
  function fork(c, distance = c.stroke) {
    const creep = c.motionModel === 'creep';
    const profile = creep ? creepAxis(distance, c.vf, c.af, c.df, c.lowVf, c.approachF) : axis(distance, c.vf, c.af, c.df);
    const factor = c.loadedFactor, lift = creep ? constant(c.transferLift ?? .11, c.lowVy, '이재 저속 승강') : axis(c.transferLift ?? .11, c.vy * factor, c.ay * factor, c.dy * factor);
  ...
  return { VERSION, port, axis, creepAxis, at, move, fork, forkAt, plan, planAt, sampleTimes };
});
