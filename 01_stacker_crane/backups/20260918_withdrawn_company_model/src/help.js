(function () {
  ...
  const entries = {
    ...
    targetUtil: ['목표 설계 부하율', '필요 대수를 산정할 때 확보할 처리 여유를 정합니다. 0.8이면 보정 처리능력의 80% 이내로 피크 요구량을 배분합니다.', '필요 대수 = 올림(총 피크 요구량 ÷ 1대 보정 능력 ÷ 목표 부하율). 포트 조건이 다르면 현재 STC 중 가장 낮은 능력을 기준으로 산정합니다.'],
    ...
    factorSection: ['분석 보정계수', '이론 처리량에 A × Fₜ × E𝑤를 적용하여 보정 능력을 계산합니다. 값과 근거가 있어야 필요 대수와 판정을 표시합니다.', '설비 사양·실측·설계 가정을 기록하세요. 시뮬레이션 결과에는 이 계수를 추가 적용하지 않습니다.'],
    ...
    overviewTab: ['물동량 계산', 'SC/DC 사이클 시간, 이론·보정 처리량, 필요 대수와 랙·포트·FEM 적용점을 확인합니다.', '먼저 왼쪽 설비·운영 조건을 입력하고 계산 및 설정 적용을 누르세요. 보정계수 미입력 시 이론값만 표시합니다.'],
    ...
    capacityResult: ['요구량과 처리능력', '전체 피크 입출고 수요와 STC 처리능력을 비교합니다. 보정계수와 근거가 있으면 목표 부하율에 따른 필요 대수도 표시합니다.', '포트가 다르면 통로별 능력이 다를 수 있습니다. 필요 대수 산정에는 현재 STC 중 가장 낮은 보정 능력을 사용합니다.'],
    sensitivityResult: ['보정계수 민감도', '기준 보정계수를 상대적으로 ±5%, ±10% 바꾸었을 때 처리능력과 필요 대수가 얼마나 달라지는지 비교합니다.', '예: 0.8의 +10%는 0.88입니다. 계수의 불확실성이 설계 판단에 주는 영향을 검토하세요.'],
  ...
  Object.assign(entries, {
    loadCompany: ['회사 입력 불러오기', 'SMC stcfem2의 87행 입력 파일을 불러옵니다. 확장자가 없어도 됩니다.', 'CASE 1, Single fork, 역번지 0, 균일 번지·단간격, 승강·포크의 초고속과 고속이 각각 같은 사양을 지원합니다. 요구량은 기존 값을 유지합니다.'],
    motionModel: ['운동시간 계산 방식', '기존 정지 간 가감속 또는 저속 접근 포함 방식을 선택합니다.', '회사식은 저속 접근거리와 이동 여유, 이재 저속 승강을 반영합니다. 최종 저속 정지는 순간 정지 근사이며 현장 실측 운동 선도와는 다를 수 있습니다.'],
    pointMode: ['대표점 위치', '실제 화물 중심은 가까운 셀로 옮깁니다. 이론 좌표는 FEM 포트 조건을 검사합니다.', '연속 대표점은 셀로 옮기지 않고 입력한 외부 포트까지의 거리를 계산합니다. 회사 입력 비교에 사용하며 3D 셀은 개략 배치입니다.'],
    lowVx: ['주행 저속', '최종 접근 구간의 주행 속도입니다. 최대속도 이하여야 합니다.'],
    lowVy: ['승강 저속', '최종 승강 접근 속도이며 회사식의 캐리지 이재 승강에도 사용합니다.'],
    lowVf: ['포크 저속', '신장·수축 끝부분 최소 접근거리의 속도입니다.'],
    approachX: ['주행 최소 접근거리', '저속으로 이동하는 마지막 거리(m)입니다.'],
    approachY: ['승강 최소 접근거리', '저속으로 이동하는 마지막 거리(m)입니다.'],
    approachF: ['포크 최소 접근거리', '포크 신장·수축의 마지막 저속 거리(m)입니다.'],
    moveAllowance: ['이동 여유시간', '최소 접근거리보다 긴 이동에서 각 축 운동시간에 추가합니다. 주행·승강 중 큰 시간을 선택하므로 이중 가산하지 않습니다.'],
    transferLift: ['캐리지 이재 승하강량', '상차·하차 시 캐리지 이동량(mm)입니다.', '기존 방식은 승강 가감속을, 저속 접근 방식은 거리÷승강 저속을 사용합니다.'],
    forkDwell: ['안착 대기 / 포크 여유', '포크 이재 1회에 한 번 적용합니다. 회사 파일의 포크 여유시간이 여기에 들어갑니다.']
  });
