(function (root) {
  ...
  const labels = { motionModel: { stop: '정지 간 가감속', creep: '저속 접근 포함 · 회사식' }, rackDepth: { single: '싱글딥', double: '더블딥' }, method: { cells: '전체 셀 평균', fem: 'FEM CASE 기준' }, pointMode: { cell: '실제 화물 중심', theory: '이론 좌표 비교', continuous: '연속 대표점 · 외부 포트' },
  ...
  function documentHtml(result, snapshot, options = {}) {
    ...
    const inputRows = keys => keys.map(key => {
      ...
      return [field?.[0] || ({ motionModel: '운동시간 계산 방식', rackDepth: '화물 보관 깊이', mode: '운전 모드', ratioBasis: '비율 기준', storage: '입고 셀 선택', retrieval: '출고 셀 선택', arrival: '작업 도착', basis: '보정 근거', method: '계산 방식', pointMode: 'P1/P2 위치' })[key] || key,
    ...
    section('1. 설비 및 동작 사양',
      ...
      '<h3>주행·승강·포크 운동</h3>' + table(['계산 방식', '값', '단위'], inputRows(['motionModel'])) + (c.motionModel === 'creep' ? table(['저속 접근', '값', '단위'], inputRows(['lowVx', 'lowVy', 'lowVf', 'approachX', 'approachY', 'approachF', 'moveAllowance'])) : '') + table(['축', '최대속도 (' + c.speedUnit + ')', '가속도 (m/s²)', '감속도 (m/s²)'],
      ...
      note(c.motionModel === 'creep' ? '캐리지 이재시간=승하강량÷승강 저속. 이동 여유는 각 축 시간에 포함됩니다. 저속 접근 끝의 정지는 순간 정지로 근사합니다.' : '승하강 중 포크는 완전 신장 상태로 정지합니다. 캐리지 이재에는 승강 속도·가감속과 적재 비율을 적용합니다. 랙·포트 Y 및 P1/P2는 놓인 화물 중심이며 적재 이동 시 이재 승하강량만큼 높아집니다.'));
    ...
    section('적용 조건 및 민감도',
      ...
      note('Single / Double Deep, Single Fork, 균등 랙, 통로당 1대입니다. jerk 제한 S-curve, 고장, 더블딥 화물 재배치·SKU 지정 출고, Twin Fork, 공유레일 충돌은 포함하지 않습니다. 현장 실측 및 규격 적합성 판정은 별도입니다.') +
      '<h3>보정계수 민감도</h3>' +
