(function (root, factory) {
  ...
  'use strict';
  const VERSION = '0.3.3', SCHEMA = 'stc/project@1';
  const defaults = {
    ...
    motionModel: 'stop', lowVx: 5 / 60, lowVy: 5 / 60, lowVf: 5 / 60, approachX: .2, approachY: .1, approachF: .08, moveAllowance: 1.5,
  ...
  const fields = {
    lowVx: ['주행 저속', 'speed', .001, 20, .01], lowVy: ['승강 저속 · 이재 속도', 'speed', .001, 10, .01], lowVf: ['포크 저속', 'speed', .001, 5, .01],
    approachX: ['주행 최소 접근거리', 'm', 0, 20, .01], approachY: ['승강 최소 접근거리', 'm', 0, 20, .01], approachF: ['포크 최소 접근거리', 'm', 0, 20, .01],
    moveAllowance: ['이동 여유시간', 's/이동축', 0, 120, .1],
  ...
  const enums = { motionModel: ['stop', 'creep'], rackDepth: ['single', 'double'], method: ['cells', 'fem'], pointMode: ['cell', 'theory', 'continuous'], mode: ['sc-return', 'sc-stay', 'dc', 'mixed'], ratioBasis: ['cycles', 'loads'], storage: ['random', 'nearest'], retrieval: ['fifo', 'random', 'nearest'],
  ...
  function migrate(input) {
    ...
    const extra = {};
    for (const k of ['motionModel', 'lowVx', 'lowVy', 'lowVf', 'approachX', 'approachY', 'approachF', 'moveAllowance']) extra[k] = input[k] === undefined ? defaults[k] : input[k];
    return { ...input, ...extra, ioZ: input.ioZ ?? null, outX: input.outX ?? null, outY: input.outY ?? null, outZ: input.outZ ?? null,
  ...
  function validate(input) {
    ...
    if (c.motionModel === 'creep') for (const [key, high] of [['lowVx', 'vx'], ['lowVy', 'vy'], ['lowVf', 'vf']]) {
      if (c[key] > c[high]) { const error = Error(fields[key][0] + '은 최대속도 이하여야 합니다.'); error.field = key; throw error; }
    }
  ...
  function femPoints(c) {
    ...
    const inbound = mode === 'cell' ? snapPair(reference.inbound) : reference.inbound;
    const outbound = c.femCase === 2 ? (mode === 'cell' ? snapPair(reference.outbound) : reference.outbound) : inbound;
  ...
  function calculate(input) {
    ...
    return { ...first, config: c, p, mix: mixing, allRoutes, demand, factor, capacity, util, worstUtil, required, theoryTotal, aisles,
      ...
      reference: (c.method === 'fem' ? 'FEM 9.851 (06.2003) CASE ' + [...new Set(allRoutes.map(a => a.femCase))].join('/') + (c.pointMode === 'cell' ? ' 기준 실제 화물 중심 · 설계 계산' : c.pointMode === 'continuous' ? ' 연속 대표점 · 설계 확장' : ' 이론 대표점 비교') + (allRoutes.some(a => !a.fem.portMatch) ? ' · 실제 외부/자유 포트 적용' : '') : '균일 셀 선택 · SC-return/DC 공학 모델')
