(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.ShuttleMotion=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  'use strict';
  ...
  function move(c,section,from,to,loaded){
    const factor=section.speedFactor??1;
    const spec={v:(loaded?c.loadedV:c.emptyV)*factor,a:loaded?c.loadedA:c.emptyA,d:loaded?c.loadedD:c.emptyD,stopDwell:c.stopDwell};
    if(c.jerkMode==='reduced'){spec.a*=c.accelScale;spec.d*=c.accelScale;}
    const zones=c.zones.map(z=>({...z}));
    if(c.creepDistance>0&&Math.abs(to-from)>EPS){const edge=to-Math.sign(to-from)*Math.min(c.creepDistance,Math.abs(to-from));zones.push({from:edge,to,v:c.creepV});}
    const profile=solve(from,to,spec,zones,c.stations.filter(s=>s.stop).map(s=>s.x));
    if(profile.duration>0&&c.jerkMode==='delay'&&c.jerkDelay>0){profile.phases.push({start:profile.duration,duration:c.jerkDelay,p:profile.length,v:0,a:0,label:'jerk 보정 대기'});profile.duration+=c.jerkDelay;}
    return profile;
  }
  return{VERSION,solve,at,move};
});
