(function (root, factory) {
  ...
  class Engine {
    constructor(input) {
      ...
      this.position = M.port(c, c.mode === 'sc-return' && c.inbound === 0 ? 'out' : 'in');
    ...
    scheduleArrivals(kind, rate, random) {
      ...
      while (t < this.horizon) {
        ...
        this.heap.push({ type: 'arrival', time: t, kind, id: kind + '-' + count }); t += gap();
      }
    }
    ...
    admit() {
      while (this.backlog.length && this.inSlots < this.config.inBuffer) { this.inQueue.push(this.backlog.shift()); this.inSlots++; }
    }
    ...
    dispatch() {
      ...
      const plan = M.plan(c, kind, this.position, inCell, outCell, c.mode === 'sc-return');
      this.cycle = { kind, inJob, outJob, inCell, outCell, plan, start: this.time };
    ...
    startAction() {
      ...
      if (action.effect === 'placeOut' && this.outputCount >= this.config.outBuffer) { this.state = 'blocked'; this.active = null; return; }
    ...
    finishJob(job, kind) {
      ...
      this.logs.push({ id: job.id, kind, arrival: job.arrival, start: job.start, end: job.end,
        ...
        cell: { ...(kind === 'in' ? this.cycle.inCell : this.cycle.outCell) }, port: M.port(this.config, kind) });
    }
    finishAction() {
      const action = this.active, cycle = this.cycle;
      this.position = { ...action.to };
      switch (action.effect) {
        case 'pickIn': this.inSlots--; this.cargo = 'in'; this.admit(); break;
        ...
        case 'placeOut':
          ...
          else { this.outputCount++; this.lastTakeaway = Math.max(this.time, this.lastTakeaway) + this.config.takeaway; this.heap.push({ type: 'takeaway', time: this.lastTakeaway }); }
          break;
      }
    ...
    advanceTo(target) {
      ...
      while (this.heap.peek() && this.heap.peek().time <= target) {
        ...
        if (event.type === 'arrival') {
          const job = { id: event.id, kind: event.kind, arrival: this.time }; this.created[event.kind]++;
          ...
          this.outputCount--; this.shipped++; if (this.time >= this.warmup) this.measuredShipped++;
  ...
  class Fleet {
    constructor(input) {
      ...
      this.engines = Array.from({ length: this.config.cranes }, (_, i) => new Engine({ ...Model.aisleConfig(this.config, i), seed: (this.config.seed + Math.imul(i, 104729)) >>> 0 }));
