(function () {
  ...
  const entries = {
    ...
    bays: ['Bay 수 · 번지수', '한 보관면에서 주행 방향으로 나열된 번지 개수입니다. 1부터 입력한 Bay 수까지 동일 간격의 셀을 생성합니다.', '전체 저장 셀 수 = Bay 수 × Level 수 × 보관면 수 × STC 수. 3D와 셀 주소도 같은 개수를 사용합니다.'],
    ...
    ioZ: ['IN · Z 포크 좌표', '각 STC 통로 중심에서 입고 포트까지 포크 방향 거리를 m로 입력합니다. A면 방향은 음수, B면 방향은 양수입니다.', '빈칸은 −포크 스트로크입니다. 예: 스트로크 1.2 m에서 Z=−1이면 포크 편도 이동은 1 m입니다. |Z|는 스트로크 이하여야 합니다.'],
    ...
    stroke: ['포크 편도 스트로크', '포크가 통로 중심에서 랙 화물 중심까지 한쪽으로 나가는 거리입니다. 단위는 m이며 OUT과 IN 각각에 이 거리를 사용합니다.', '랙 화물의 Z는 A면 −스트로크, B면 +스트로크입니다. IN/OUT 포트는 개별 |Z|만큼 움직이며 스트로크 이내여야 합니다.'],
    ...
    viewFront: ['정면 보기', '전체 랙과 STC를 정면 방향으로 보도록 카메라를 맞춥니다.', '번지와 단 위치를 확인할 때 사용합니다. 휠로 확대하고 우클릭 드래그로 이동할 수 있습니다.'],
    viewIso: ['입체 보기', '전체 STC와 양쪽 랙이 보이는 입체 시점으로 카메라를 맞춥니다.', '드래그로 회전, 휠로 확대, 우클릭 드래그로 이동할 수 있습니다.'],
  ...
  function mountPorts() {
    ...
    headers.forEach((header, index) => {
      const key = index ? 'individual-' + STCModel.portKeys[index - 1] : 'portsTable';
      if (index) {
        const source = entries[STCModel.portKeys[index - 1]];
